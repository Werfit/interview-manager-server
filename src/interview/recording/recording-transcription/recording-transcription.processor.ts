...
import { EmbeddingDatabaseService } from 'src/database/embedding-database/embedding-database.service';
import { EmbeddingService } from 'src/embedding/embedding.service';
import { tryCatch } from 'src/shared/utilities/try-catch/try-catch.utility';
import { ITranscriptionSegment } from 'src/transcription/providers/providers.types';
...
export class RecordingTranscriptionProcessor extends WorkerHost {
  ...
  async process(job: Job<RecordingTranscriptionJobData>) {
    const { segments, interviewId } = job.data;
    ...
    const [success] = await tryCatch(async () => {
      const embeddingCollection =
        await this.embeddingDatabaseService.getOrCreateCollection(interviewId);

      await this.embeddingDatabaseService.addEmbeddings(embeddingCollection, {
        ids: chunks.map((_, index) => `${interviewId}-${index}`),
    ...
    this.eventBus.publish(new TranscriptionEmbeddedEvent(interviewId));
  }
